...
Function FProcesaDaot(hParam)
    ...
    IF !empty(vHoja)
        ...
        For nRow:=7 to 21
            For nCol:=3 to 7 
                xcampo:=oHoja:Cells(nRow,1):Value + oHoja:Cells(5,nCol):Value            
                IF !empty(xcampo)
                    ...
                    IF posi > 0
                        ...
                        IF nCol<6 .and. vdato<>nil
                            valor[posi]:=vdato
                        Else									
                            IF vdato<>Nil
                                if vdato>0
                                    valor[posi]:=Alltrim(str(vdato*100,6,2))
                                endif
                            Endif
                        Endif                                    
                    Endif
